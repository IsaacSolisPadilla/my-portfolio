<template>
  ...
  >
    ...
    <div class="flex flex-col max-w-xl text-indigo-100">
      <h2
        class="text-4xl font-extrabold mb-3 tracking-wide text-indigo-400"
        v-intersect.once="fadeIn"
      >
        Sobre mí
      </h2>
      <h3
        class="text-xl font-semibold mb-6 text-indigo-300"
        v-intersect.once="fadeIn"
      ...
      <p
        class="text-lg leading-relaxed mb-8 text-indigo-200"
        v-intersect.once="fadeIn"
        style="animation-delay: 300ms"
      >
        Soy Isaac Solis Padilla, apasionado por la tecnología y el desarrollo de software de alta calidad.
        Me especializo en crear soluciones escalables y eficientes con las mejores prácticas y tecnologías modernas.
      </p>
      ...
      >
        Descargar CV
        <span
          class="ripple absolute rounded-full bg-indigo-300 opacity-30 pointer-events-none"
          ref="ripple"
        ></span>
      </button>
    </div>
...
export default {
  ...
  methods: {
    downloadCV() {
      // Aquí podrías poner la url real de tu CV o abrir modal
      alert('CV descargado! (aquí pon el enlace real)');
    },
    fadeIn(el) {
      el.style.opacity = 0;
